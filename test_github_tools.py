"""Test GitHub MCP tools using FastMCP in-memory client."""
import asyncio
import sys
import os
...
async def test_list_user_repos():
    """Test list_user_github_repos tool."""
    print("\n" + "="*70)
...
async def test_search_repos():
    ...
    print("\n" + "="*70)
    print("Testing search_github_repos")
    print("="*70)
    
    async with Client(mcp) as client:
        print("\n🔍 Testing with query='RAG user:aviciot', limit=5")
        result = await client.call_tool(
            "search_github_repos",
            arguments={
...
async def main():
    """Run all tests."""
    print("\n" + "="*70)
    print("🧪 GitHub MCP Tools Test Suite")
    print("="*70)
