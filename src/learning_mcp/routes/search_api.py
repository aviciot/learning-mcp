# /src/learning_mcp/routes/search_api.py
...
# Extract likely API paths like /api/... or /public/...
_PATH_RE = re.compile(r"(/(?:api|public)[^\s\"'()<>]+)")
# Extract key=value style tokens often found in docs (very light)
...
def _build_vdb(profile_name: str) -> VDB:
    ...
    return VDB(url=vcfg.get("url"), collection=vcfg.get("collection"), dim=dim, distance=(vcfg.get("distance") or "cosine"))

# ---- Hints helpers (best-effort) ----
...
def _method_hint_from_text(text: str) -> Optional[str]:
    if not text:
        return None
    # very light-weight: look for typical verbs
    low = text.lower()
...
# ---- Route ----
@router.post("/search/api_context", name="api_context", response_model=SearchResponse, tags=["search"])
async def api_context(body: SearchRequest = Body(...)) -> SearchResponse:
    """
    Dense semantic search over the profile's collection.
    - Primary filter: payload.doc_id == profile
    - Legacy fallback: payload.profile == profile
    Returns minimal fields + optional 'hints' used by api_plan/api planners.
    """
